...
constexpr uint8_t LIS3MDL_WHOAMI = 0x3D;
...
bool gciLIS3MDL::reboot() { return Write(REG_CTRL_REG1, 0x01, 1, 3); }
...
bool gciLIS3MDL::setBdu(bool en) {
  return Write(REG_CTRL_REG5, (en) ? BDU_ON : BDU_OFF, 1, 6);
}

bool gciLIS3MDL::shutdown() {
  return Write(REG_CTRL_REG3, OP_MODE_POWERDOWN, 2, 0);
}

bool gciLIS3MDL::enableTemp() { return Write(REG_CTRL_REG1, 0x01, 1, 7); }

bool gciLIS3MDL::setPerformanceMode(const PerfMode perf_mode) {
  if (!Write(REG_CTRL_REG1, perf_mode, 2, 5))
    return false; // x y axes
  if (!Write(REG_CTRL_REG4, perf_mode, 2, 2))
    return false; // z axis
  if (!Write(REG_CTRL_REG3, OP_MODE_CONTINUOUS, 2, 0))
