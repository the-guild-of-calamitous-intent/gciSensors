...
class PA1010D : public SensorI2C {
  // const int addr = 0x10;
  // const uint32_t MAX_READ{250};
  uint8_t buffer[MAX_READ]{0};

public:
  PA1010D(const uint8_t addr = PA_ADDR, uint32_t port = 0)
      : SensorI2C(addr, port) {}
  ...
  // void parse_string(char output[], char protocol[]) {
  ...
  //     // Displays GNRMC data
  //     // Similarly, additional if statements can be used to add more
  //     protocols if (strcmp(protocol, "GNRMC") == 0) {
  //         printf("Protcol:%s\n", gps_data[0]);
  //         printf("UTC Time: %s\n", gps_data[1]);
  //         printf("Status: %s\n", gps_data[2][0] == 'V' ? "Data invalid. GPS
  //         fix not found." : "Data Valid"); printf("Latitude: %s\n",
  //         gps_data[3]); printf("N/S indicator: %s\n", gps_data[4]);
  //         printf("Longitude: %s\n", gps_data[5]);
  //         printf("E/W indicator: %s\n", gps_data[6]);
  //         printf("Speed over ground: %s\n", gps_data[7]);
  //         printf("Course over ground: %s\n", gps_data[8]);
  //         printf("Date: %c%c/%c%c/%c%c\n", gps_data[9][0], gps_data[9][1],
  //         gps_data[9][2], gps_data[9][3], gps_data[9][4],
  //               gps_data[9][5]);
...
// !defined(PICO_DEFAULT_I2C_SCL_PIN) #warning i2c/mpu6050_i2c example requires
// a board with I2C pins
...
//     Pico) i2c_init(i2c_default, 400 * 1000);
